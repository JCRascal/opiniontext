...
test_that("op_author correctly identifies the author of each pdf sub-document", {
  test_in <- opauth_h2(pdftools::pdf_text(system.file("extdata", "19-631_2d93.pdf", package = "opiniontext")))

  known <- c("Syllabus", "Kavanaugh", "Sotomayor", "Breyer", "Gorsuch")

  expect_identical(op_author(test_in), known)

  test_in <- opauth_h2(pdftools::pdf_text(system.file("extdata", "18-938_l6gn.pdf", package = "opiniontext")))

  known <- c("Syllabus", "Ginsburg")

  expect_identical(op_author(test_in), known)

  test_in <- opauth_h2(pdftools::pdf_text(system.file("extdata", "18-725_f2bh.pdf", package = "opiniontext")))
